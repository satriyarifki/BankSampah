    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transaksis.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('penyetor_id') ? 'is-invalid' : '' }}" name="penyetor_id" id="penyetor_id"  required>
                    @foreach($penyetor as $id => $penyetor)
                        <option value="{{ $id }}" {{ old('penyetor_id', '' ) ? 'selected' : '' }}>{{ $penyetor }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_hp') ? 'is-invalid' : '' }}" type="number" name="bayar" id="bayar" value=" {{ old('bayar', '') }}" required>
                @if($errors->has('jumlahsampah'))
                    <div class="invalid-feedback">
                        {{ $errors->first('jumlahsampah') }}
                    </div>
